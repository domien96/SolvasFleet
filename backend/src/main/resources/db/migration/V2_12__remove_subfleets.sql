...
UPDATE fleet_subscriptions f SET fleet_id = (SELECT sf.fleet_id FROM subfleets sf JOIN fleets USING (fleet_id) WHERE sf.subfleet_id = f.subfleet_id ) WHERE subfleet_id >= 1;
