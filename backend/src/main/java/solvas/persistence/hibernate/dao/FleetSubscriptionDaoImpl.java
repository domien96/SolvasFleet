...
public class FleetSubscriptionDaoImpl implements FleetSubscriptionDaoCustom {
    ...
    public Optional<FleetSubscription> activeForVehicle(Vehicle vehicle) {
        return Optional.ofNullable((FleetSubscription) entityManager.createQuery("select s from FleetSubscription s where s.vehicle = ?1 and s.startDate <= current_date and (s.endDate is null or s.endDate >= current_date) and s.archived = false").getSingleResult());
    }

    @Override
    public Collection<FleetSubscription> findByFleetAndStartDateAndEndDate(Fleet fleet, LocalDate start, LocalDate end) {
        return (Collection<FleetSubscription>) entityManager.createQuery("select s from FleetSubscription s where s.fleet = ?1 and (s.endDate is null or s.endDate >= ?2) and s.startDate <= ?3 and s.archived = false").getResultList();
    }
}
