...
test('Fleet renders correctly', () => {
  const fleet = mount(<Fleet params={ { id: 1, companyId: 2 } }/>);
...
test('Subfleet renders correctly', () => {
  const subfleet = mount(<Subfleet type='A' vehicles={ [{ id: '1', type: 'A' }] } isChecked={ jest.fn() } isIndeterminate={ jest.fn() } showVehicles={ true }/>);
