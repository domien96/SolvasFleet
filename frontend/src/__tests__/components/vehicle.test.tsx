...
jest.mock('../../javascripts/actions/vehicle_actions.ts', () => ({
  fetchVehicle: jest.fn().mockImplementation((a,b)=>b({"id":"1","licensePlate":"AA","brand":"volvo","vin":"EE","model":"R",
  "type":"T","mileage":"1000","year":"1990","leasingCompany":"1","fleet":"2","value":"3000"}))
}));
...
test('VehicleView renders correctly',()=>{
  ...
  expect(view.find('DetailTable').prop('data')).toEqual(
    ["fleet","vin","licensePlate","brand","model","type","mileage","year","value","leasingCompany"].map((d)=>{
      return {key:"vehicle."+d,label:veh[d]};
    })
  );
})
