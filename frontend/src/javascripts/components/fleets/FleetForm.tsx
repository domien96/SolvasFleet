  ...
  return (
    <form method='post' onSubmit={ props.onSubmit } className='fleet-form'>
      ...
      <span>
        ...
        <FormChoice
          placeholder='Facturation Period'
          choices={[
            th(3, 'fleet.options.perTrimester'),
            th(6, 'fleet.options.perSemester'),
            th(12, 'fleet.options.yearly'),
          ]}
          value={ (props.fleet.facturationPeriod || '').toString() }
          callback={ handleChange('facturationPeriod') } />
        <FormChoice
          placeholder="Payment Period"
          choices={[
            th(1, 'fleet.options.monthly'),
            th(3, 'fleet.options.perTrimester'),
            th(6, 'fleet.options.perSemester'),
            th(12, 'fleet.options.yearly'),
          ]}
          value={ (props.fleet.paymentPeriod || '').toString() }
          callback={ handleChange('paymentPeriod') } />
      </span>
